...
export function CSVExport() {
  ...
  useEffect(() => {
    async function fetchCollections() {
      ...
      if (error) {
        ...
        setCollections(cols);
        // Auto-select first collection if only one exists
        if (cols.length === 1) {
  ...
  // Fetch vehicles when collection is selected
  useEffect(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      {!selectedCollectionId && !loadingVehicles && (
        <div className="text-center py-4 text-muted-foreground">
          Select a collection to see export options
        </div>
      )}

      {/* No vehicles in collection */}
      {selectedCollectionId && !loadingVehicles && vehicles.length === 0 && (
        <div className="text-center py-8">
          <p className="text-muted-foreground mb-4">No vehicles in this collection.</p>
          <a
            href="/vehicles/new"
            className="text-primary hover:underline"
          >
      ...
      {/* Stats and Export Options - only show when collection has vehicles */}
      {selectedCollectionId && !loadingVehicles && vehicles.length > 0 && (
        <>
          <div className="bg-card border border-border p-4">
